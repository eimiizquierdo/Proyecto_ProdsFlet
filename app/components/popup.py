...
async def confirm_dialog(page: ft.Page, title:str, message:str, function_on_yes, function_on_cancel, bgcolor:str=Colors.DANGER, txtcolor:str=Colors.WHITE):
    dlg = ft.AlertDialog(
        modal=True,
        title_padding=0,
        content_padding=0,
        ...
        actions=[ft.TextButton("Cancelar",on_click=lambda e: _close_confirm_dialog(page, function_on_cancel)),
                 ft.Button("Si, borrar", on_click=lambda e: _yes(page, function_on_yes))], 
                 actions_alignment=ft.MainAxisAlignment.END
    )
    page.show_dialog(dlg)

def _close_confirm_dialog(page: ft.Page, function_on_cancel):
    pop = getattr(page,"pop_dialog",None)
    if callable(pop):
        page.pop_dialog()
    page.update()
